<!-- footer -->
<footer class="" id="footer1">
  <div class="container-fluid">
    <div class="row">
      <div class="col l4 offset-l2 s12" id="Menufooter">
        <h5 class="">Menu</h5>
        <ul>
          <li><a class="" href="index.php?page=home">Accueil</a></li>
          <li><a class="" href="index.php?page=sortie">Les bons plans sorties!</a></li>
          <li><a class="" href="index.php?page=profil">Je partage !</a></li>
          <li><a class="" href="#">Contact</a></li>
        </ul>
      </div>
      <div class="col l6 s12" id="textfooter">
        <h5 class="">LH KIDS</h5>
        <p class="">Site d'échange et de partage de bons plans sorties entre particuliers au Havre et ses alentours.</p>
      </div>
      <div class="mentions">
        <a class="" href="index.php?page=mentionslegales">Mentions légales LH Kids</a>
      </div>
    </div>
    </div>
    <div class="container-fluid" id="footer2">
      <div id="bottomFooter" class="row">
      <div class="col s6 l4 offset-l7">
          <a class="grey-text text-lighten-4 right" href="#"><img class="logofooter" src="../images/logo.png" /></a>
      </div>
    </div>
    </div>
</footer>
